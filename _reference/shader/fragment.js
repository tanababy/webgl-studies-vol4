...
void main() {
    ...
    float dist = distance(u_mouse * aspect, vUv * aspect) + n;
    float range = map(dist, 0.1, radius, 0.0, 1.0);
    range = pow(range, 3.0);

    // vec2 refractPower = (uv - 0.5);
    vec2 refractPower = (uv - u_mouse);
    ...
    float limit = smoothstep(radius - 0.03, radius, dist);
    vec3 color = mix(vec3(0.0), tex.rgb, shadow);
    color = mix(inCircle, color, limit);

    gl_FragColor = vec4(color, 1.0);

    // gl_FragColor = vec4(refractPower, 0.0, 1.0);
}
